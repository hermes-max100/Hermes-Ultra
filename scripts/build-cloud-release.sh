...
if [[ "${HERMES_PRODUCTION_BUILD:-0}" == "1" ]]; then
  ...
  HERMES_EXPORT_DEPENDENCY_LOCKS=1 bash "$ROOT_DIR/scripts/stage-hermes-agent-source.sh" "$HERMES_AGENT_SOURCE_DIR" "$STAGE/vendor/hermes-agent/0.21.0"
  bash "$ROOT_DIR/scripts/stage-hermes-relay-source.sh" \
    "$HERMES_RELAY_SOURCE_DIR" \
    "$HERMES_RELAY_SERVER_WHEEL" \
    "$STAGE/vendor/hermes-relay/server-v1.11.1"
fi

...
if PATH="$BIN:$PATH" FAKE_TS_STATE=NeedsLogin FAKE_TS_LOG="$TMP/ts.log" bash "$ROOT_DIR/scripts/configure-tailscale-hermes.sh" >/dev/null 2>&1; then echo 'logged-out Tailscale accepted' >&2; exit 1; fi
PATH="$BIN:$PATH" FAKE_TS_LOG="$TMP/ts.log" bash "$ROOT_DIR/scripts/configure-tailscale-hermes.sh" >/dev/null
